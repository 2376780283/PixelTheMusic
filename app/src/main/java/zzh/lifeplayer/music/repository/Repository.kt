...
import zzh.lifeplayer.music.db.*
import zzh.lifeplayer.music.fragments.search.Filter
import zzh.lifeplayer.music.model.*
import zzh.lifeplayer.music.model.smartplaylist.NotPlayedPlaylist
import zzh.lifeplayer.music.network.LastFMService
import zzh.lifeplayer.music.network.Result
import zzh.lifeplayer.music.network.Result.Error
import zzh.lifeplayer.music.network.Result.Success
import zzh.lifeplayer.music.network.model.LastFmAlbum
...
class RealRepository(
    ...
    override suspend fun playlistSongs(playlistWithSongs: PlaylistWithSongs): List<Song> =
        playlistWithSongs.songs.map { it.toSong() }
    ...
    override fun observableHistorySongs(): LiveData<List<Song>> =
        roomRepository.observableHistorySongs().map { it.fromHistoryToSongs() }
    ...
    override suspend fun favoritePlaylistHome(): Home {
        val songs = favoritePlaylistSongs().map { it.toSong() }
